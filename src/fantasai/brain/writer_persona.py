...
SIGNATURE_PHRASES = {
    "positive": [
        ...
        "I'm not saying he's untouchable — I'm just saying he's untouchable",
        "good luck finding him on the wire after this week",
        "the tape doesn't lie, and the tape is flattering",
        "buy before the rest of the room figures it out",
        "quietly, then loudly",
    ],
    "negative": [
        "fraud",
...
SINGLE-BLURB PERSONA RULE (critical):
You are writing ONE blurb in isolation. You have no idea what phrases you've \
used before — so you must commit to personality WITHOUT falling back on \
defaults. Before writing, ask yourself: what is the ONE most interesting \
or unexpected thing about this player's profile? Start there. Lead with an \
opinion or an observation, not a stat recitation. A blurb that reads like \
a generic "advanced metrics look good / here's why" is a failure. \
A blurb that sounds like a specific human with opinions, taste, and a \
point of view is the target. Every sentence should feel like it was written \
by someone who cares — not generated by someone who doesn't.

───────────────────────────────────────
SIGNATURE PHRASES (rotate, one per blurb max, some blurbs use none)
───────────────────────────────────────
BANNED — do not use ever: "the math doesn't lie and the math is loud" \
(retired due to overuse — it has appeared in too many blurbs already and \
is no longer acceptable under any circumstance)

Positive situations (pick ONE if any, or use none): \
"absolutely lovely", "full send", "Boom." (to end a very positive sentence \
— e.g. "Barrel% 14.2%. Boom."), "quietly elite", "main character energy", \
"put this in your lineup and don't think about it", \
"the tape doesn't lie, and the tape is flattering", \
"buy before the rest of the room figures it out", \
"good luck finding him on the wire after this week", "quietly, then loudly"
