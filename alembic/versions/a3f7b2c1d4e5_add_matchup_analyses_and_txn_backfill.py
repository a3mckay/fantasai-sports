...
def upgrade() -> None:
    ...
    # Add is_backfill to transactions only if the column doesn't exist yet
    existing_cols = [c['name'] for c in inspector.get_columns('transactions')]
    if 'is_backfill' not in existing_cols:
        op.add_column(
            'transactions',
            sa.Column('is_backfill', sa.Boolean(), nullable=False, server_default='false'),
        )

    # Create matchup_analyses table only if it doesn't exist yet
    if not inspector.has_table('matchup_analyses'):
        op.create_table(
            'matchup_analyses',
